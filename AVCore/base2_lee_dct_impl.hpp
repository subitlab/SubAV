...
namespace SubIT::base2_lee_dct_impl {
    ...
    struct butterfly_split_forward {
        template <size_t i>
        static constexpr void for_operator(volatile const Ty* const vec, volatile Ty* const tmp) {
        ...
        constexpr void operator()(volatile const Ty* const vec, volatile Ty* const tmp) {
    ...
    struct butterfly_merge_forward {
        template <size_t i>
        static constexpr void for_operator(volatile Ty* const vec, volatile const Ty* const tmp) {
            vec[(i << 1) + 0] = tmp[i];
            vec[(i << 1) + 1] = tmp[i + (N >> 1)] + tmp[i + (N >> 1) + 1];
        }
        constexpr void operator()(volatile Ty* const vec, volatile const Ty* const tmp) {
    ...
    struct make_sequence_orthogonal {
        ...
        static constexpr void for_operator(Ty* const vec) {
            if constexpr (i != 0) { vec[i] *= std::numbers::sqrt2_v<Ty>; }
            vec[i] /= compile_time::sqrt_v<Ty, static_cast<Ty>(N)>;
        }
    ...
    template <size_t N, typename Ty>
    constexpr void forward_transform_recursive_part(volatile Ty* const x, volatile Ty* const t) {
        if constexpr (N != 1) {
    ...
    struct butterfly_split_inverse {
        template <size_t i>
        static constexpr void for_operator(volatile Ty* const vec, volatile Ty* const tmp) {
            tmp[i] = vec[i << 1];
            tmp[i + (N >> 1)] = vec[(i << 1) - 1] + vec[(i << 1) + 1];
        }
        constexpr void operator()(volatile Ty* const vec,  volatile Ty* const tmp) {
    ...
    template <size_t N, typename Ty>
    constexpr void inverse_transform_recursive_part(volatile Ty* const x, volatile Ty* const t) {
        if constexpr (N != 1) {
